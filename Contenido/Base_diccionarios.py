...
print(operaciones["A007"][-1])

## Crear una funcion donde le pasa la lista como parametro
## Esta funcion debe mostrar todos los datos del diccionario

def muestraAutos(diccio):
    for codigo, valores in diccio.items():
        print(f"{codigo}.- {valores}")

muestraAutos(autos)


## Discriminar valores segun ciertos parametros, en este caso, si estan vendidos

def vendidos(diccio):
    for codigo, valores in diccio.items():
        if operaciones[codigo][-1]!="Pendiente":
            print(f"{codigo}.- {valores} Fecha venta: {operaciones[codigo][-1]}")
print("-"*50)
vendidos(autos)


## Crear funcion para agregar vehiculos
## Tiene que considerar agregar un registro
## a la coleccion operaciones
...
def agregarAuto(dic):
    ...
    operaciones[codigo]=[time.strftime("%d-%m-%Y"), "Pendiente"]
